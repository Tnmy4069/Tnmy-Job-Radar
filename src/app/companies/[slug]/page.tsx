...
export default function CompanyDetailPage() {
  ...
  async function load(nextView = view) {
    const data = await fetch(`/api/companies/${params.slug}?view=${nextView}`).then((r) => r.json());
  ...
  async function scanCompany() {
    ...
    const res = await fetch("/api/scan", {
      ...
      body: JSON.stringify({ company: params.slug }),
    }).then((r) => r.json());
  ...
  async function toggle() {
    if (!company) return;
    await fetch(`/api/companies/${params.slug}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
